...
const FormattedTime = ({ time, small }) => {
  ...
  return (
    <p className={`${small ? 'font-bold text-2xl' : 'font-light text-[4.5rem]'}`}>
      {minutes}:{seconds}
...
const LapTime = ({time}) =>{
  return (
    <div className='w-full flex items-center justify-between bg-background-transparent backdrop-brightness-200 px-4 py-2 rounded-3xl'>
  ...
  return (
    <section className="w-full h-full p-4 flex flex-col md:flex-row items-center gap-6 bg-[url('/images/bg.webp')] bg-cover bg-center bg-no-repeat scaling-bg">
      ...
      <div className="lap-records w-full md:w-2/5 h-2/5 md:h-full border-2 border-purple-600 bg-black bg-opacity-50 backdrop-blur-sm rounded-3xl shadow-2xl order-2 md:order-1    p-4 pb-0 flex flex-col gap-2">
        ...
        <hr />
        <div className="w-full h-full overflow-y-scroll flex flex-col gap-2" ref={lapsRef}>
          {laps.slice().reverse().map((lapTime, index) =>(
            <LapTime key={index} time={lapTime} />
          ))}
        </div>
      </div>
    </section>
